...
The triad census tabulates different types of triangles or triples
...
There are 6 triples, with one tie. For example the *subgraph*
...
**Open triad**: There are 3 triples, with two ties. For example the
...
**Closed triad**: There is 1 triple, with three ties. For example the
...
For triplets in directed networks, there many different kinds. For
...
this is the total number on (un-ordered) triplets, the number of ways
...
We are going to use real networks to investigate how much these (undirected) networks
...
Here we are going to use two networks. If you want to try this out on other data, you will find how to read in the data and format it in  the `Data-Formatting-2025.Rmd`
vignette.

### Padgett business

The adjacency matrix for the @padgett1993robust business network is stored as a regular text file `PADGB.txt` on GitHub

![Files on GitHub repos](https://raw.githubusercontent.com/johankoskinen/CHDH-SNA/main/images/githubraw.png)

Read this in using `read.table`

```{r}
padgettbus <- as.matrix(read.table("https://raw.githubusercontent.com/johankoskinen/CHDH-SNA/main/data/PADGB.txt"))
# Look at the matrix - what does it look like
```

Format this as a `network` object

```{r}
padgbus.net <- as.network(padgettbus,# take the raw data
                          directed = FALSE )# make sure you tell network that this is undirected
```
...
max.deg <- 16 # just set the maximal degree to plot
par( mfrow  = c(1,3) )
...
obs.degrees <-  degree(padgbus.net,cmode='indegree')# observed degrees
obs.degree <-  c(sum(obs.degrees==0),tabulate(obs.degrees, nbins=max.deg))# observed degree distribution
...
lines(c(0:(max.deg)),obs.degree ,col='grey',lwd=3)

triads <- triad.census( g100 , mode='graph')[,4]# CHANGE
T4 <- triad.census(padgbus.net)[4]# observed number of closed triangles
hist(triads ,
     xlim= range( triads , T4 ),
     main = 'closed triads')
abline( v=T4,col='red' )# the observed density for reference
...
cg100 <- rgnm(n =100,# number of networks to generate
            nv=  dim(padgettbus)[1],#size of the network
            m = sum(padgettbus)/2,# number of edges
            mode="graph") # undirected (graph) or directed (digraph)
```
...
obs.degrees <-  degree(padgbus.net,cmode='indegree')# obserrved degrees
obs.degree <-  c(sum(obs.degrees==0),tabulate(obs.degrees, nbins=max.deg))# observed degree distribution
...
cg100 <- rgnm(n =100,# number of networks to generate
            nv= dim(tribesPos)[1],#size of the network
            m = sum(tribesPos)/2,# number of edges
            mode="graph") # undirected (graph) or directed (digraph)
```
...
```{r uplotother}
par(mfrow=c(1,2))
plot(as.network( tribesPos , directed=FALSE))
plot(as.network(cg100[1,,],directed=FALSE ))
```
...
obs.degrees <-  degree(tribesPos.net ,cmode='indegree')# obserrved degrees
obs.degree <-  c(sum(obs.degrees==0),tabulate(obs.degrees, nbins=max.deg))# observed degree distribution
degrees <-degree(cg100,g=c(1:100), cmode='indegree')# you can calculate the degree distributions for all graphs
deg.sist <- cbind( matrix( colSums(degrees==0),100,1),
                  t(apply(degrees,2,function(x) tabulate(x, nbins=max.deg) ) ) )# when tabulating we need to add the number of isolates 
...
T4 <- triad.census(tribesPos.net)[4]# observed number of closed triangles
triads <- triad.census( cg100 , mode='graph' )[,4]
hist(triads ,
     xlim= range( triads , T4),
     main = 'closed triads')
...
ber100 <- rgraph(dim(padgettbus)[1],#size of the network
            m=100,# number of networks to generate
            tprob=gden(padgettbus),# tie probability equal to observed density
            mode="graph") # undirected (graph) or directed (digraph)
```
...
You could try to see how $X \thicksim Bern(p)$ performs for some other networks. In addition to the tribes  [@hage1983structural], you could check these ones out

```{r}
### Padgetts marriage data
...
> The simulated networks look like pretty faithful representations of the
> observed network
...
T3 <- triad.census(padgbus.net)[3]# observed number of closed triangles
triads <- triad.census( g.udegs ,mode="graph" )[,3]
hist(triads ,
     xlim= range( triads , T3 ),
     main = 'open triads')
abline( v=T3,col='red' )# the observed density for reference
...
Let us try an ERGM for Kapferer's tailors. 
...
> Note that while the Social Networks module was discontinued after 2004, it was revived in 2024. Go to the NORC data explorer and check, e.g. `numgiven`
