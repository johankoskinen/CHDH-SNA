...
Assume now that we have the one-mode network for the s50 dataset
...
and that we model only the **onset** of smoking

```{r}
smoke <-  t(apply(smoke, 1, cumsum)) # for each time-point at least 1 if previous time smoker
smoke <- ( smoke>0 )+0
```
...
smoker <- sienaDependent( smoke , type = "behavior" )
```

## Create data and effects

### Data

```{r}
s50data.diffusions <- sienaDataCreate( friendship, smoker  )
```
...
```{r}
s50.effects.diff <- getEffects( s50data.diffusions )
s50.effects.diff <- includeEffects( s50.effects.diff,# again, 'adding' to the effects
                                    egoX,# sender effect for smoker
                                    #egoSqX,# non-linear sender effect
                                    altX,# receiver effect for smoker
                                   #egoXaltX,
                                    name = 'friendship',
                                    interaction1 = "smoker" # DV works the same as an IV on another DV
                )
```

The diffusion effects need to be specified as **rate effects**, because we are modelling the time till onset:

```{r}
s50.effects.diff <- includeEffects( s50.effects.diff,# again, 'adding' to the effects
                                   type='rate',# Note: Survival analysis-type-model
                                   name="smoker" ,
                                    totExposure,
                                    interaction1 = "friendship"  # DV works the same as an IV on another DV
                )
```

### Estimate

We specify everything as before

```{r, eval = FALSE}
s50.algo.diff<- sienaAlgorithmCreate( projname = 's50_diff' )
s50.est.diff <- siena07( s50.algo.diff,
                         data = s50data.diffusions,
                         effects = s50.effects.diff,
                         batch = TRUE)
```
...
```{r, eval =FALSE}
install.packages("~/multiSiena_1.2.36.tar.gz", repos = NULL, type="source")
```
...
In the `CHDH-SNA` repository, you will also find `CHDH-SNA-HSAOM.Rmd`
